...
alties<-tibble(gene=colnames(adjacency)) %>%
  left_join(positions[,c("gene","gene_ID")])%>%
...
edgeFile = "0.10_net_redo.txt",
nodeFile = "0.10_nodes_redo.txt",
weighted = T,
threshold = 0.10,
...
corall<-cor(datExpr0)[c("VC0383","VC0384","VC0385","VC0386","VC0388"),c("VC0383","VC0384","VC0385","VC0386","VC0388")]%>%
  ...
  mutate(gene=factor(gene, levels=c("VC0383","VC0384","VC0385","VC0386","VC0388")))
  
corheat<-ggplot(corall,aes(x=Sample,y=rev(gene),fill=corr))+
  geom_tile()+
  theme_cowplot(12)+
    theme(
          axis.title = element_blank(),
...
save_plot("Figures/Fig3.pdf",
          plot_grid(ballsy,
                    plotsy,
                    nrow=2,
                    labels=c("A Virulence co-expression cluster","B Binding affinity for virulence Tx factors at locus VCA0094-96"), 
...
pizza_totes <- node_df %>%
  ...
  mutate(x_pos=c(0,1,2,3,4,
         ...
         y_pos=c(rep(9,5),
                 ...
                 rep(1,5),
                 rep(0,4)),
         bettername=dynamicColors)

pizza<-node_df %>%
  group_by(dynamicColors,hypno2,hypno) %>%
  summarise(nsub=n()) %>%
  ungroup%>%
  full_join(pizza_totes)%>%
mutate(psub=nsub/ptotal)%>%
  mutate(hypno2=factor(hypno2,levels=c("Known","Important\nUnknown","Unknown")),
         genes=gsub("Unknown|Important\nUnknown","",hypno2),
         genes=gsub("Known","genes",genes),
         ptoes=case_when(hypno2=="Known" ~
                           as.character(paste0(ptotal,
                                               " genes")),
                          hypno2!="Known" ~ "")) %>%
  mutate(end_angle=2*pi*nsub/ptotal,
...
setorder(pizza,-hypno2)

pie_chart<-ggplot(data=pizza)+
  geom_arc_bar(aes(x0=x_pos,y0=y_pos,r0=0,r=0.35,amount=nsub,fill=hypno2,colour=hypno2),stat="pie")+
  geom_text(aes(label=bettername,x=x_pos,y=y_pos+0.55),size=5)+
  geom_text(aes(label=ptoes,x=x_pos,y=y_pos+0.435))+
  scale_fill_manual(values=c("skyblue2",
                             "#DE2D26",
                             "#FEE0D2",
                             "black"),
              labels=c("Known","Unknown Important","Unknown Other"),
              name="Function")+
  scale_colour_manual(values=c("skyblue2",
                             "#DE2D26",
                             "#FEE0D2",
                             "black"),
              labels=c("Known","Unknown Important","Unknown Other"),
  ...
  theme(legend.title=element_blank(),
        ...
        axis.ticks = element_blank(),
        axis.text = element_blank(),
        legend.position=c(0.875,0.17),
        legend.key = element_rect(color = NA, fill = NA),
        legend.key.size = unit(1, "cm"))
save_plot("Figures/pie_chart.pdf",pie_chart,base_height=7,base_aspect_ratio = 1.2)
...
legz <- ggraph(facetted%>%filter(!duplicated(annotation) | !duplicated(hypno2)),layout = "lgl") +
  ...
  geom_node_text(aes(label=facetted_labs),size=2.75,repel=T)+
  scale_shape_manual(name="Function",values=c(23,21,22))+
  # facet_nodes(~factor(dynamicColors,
  #                      levels=c(
  #                               "salmon",
  #                               "midnightblue",
  #                               "grey60",
...
rpos_over<-rpos_corr%>%inner_join(rpos_in)
